...
{ 
  ...
  for(i=0;i<HIS_HIGO_GODEC_NUM;i++)
  {
    ...
    SrcDesc.SrcInfo.pFileName =  pDev->imgDev[i].decFileName;
    WV_CHECK_RET( HI_GO_CreateDecoder(&SrcDesc, &(pDev->imgDev[i].decHndl)) );  
    WV_CHECK_RET( HI_GO_DecCommInfo(pDev->imgDev[i].decHndl, &(pDev->imgDev[i]. decInfo)));
    WV_CHECK_RET( HI_GO_CreateSurface(pDev->imgDev[i].decInfo.ScrWidth, pDev->imgDev[i].decInfo.ScrHeight, HIS_HIGO_PIXELFORMAT, &(pDev->imgDev[i].bkSurfHndl)));  
    pDev->imgDev[i].dstSurfHndl = pDev->layerSurfHndl; 
    } 
  */
  return  WV_SOK;
...
WV_S32 HIS_GO_Close(HIS_DEV_HIGO_E * pDev)
{
  ...
  for(i=0;i<HIS_HIGO_GODEC_NUM;i++)
  {
    WV_CHECK_RET( HI_GO_DestroyDecoder(pDev->imgDev[i].decHndl));
    WV_CHECK_RET( HI_GO_FreeSurface(pDev->imgDev[i].bkSurfHndl));
  } 
  */ 
...
WV_S32 HIS_GODEC_Open(HIS_DEV_GODEC_E * pDev,WV_S8 *pFileName,HI_HANDLE  dstSurfHndl)
{
   ...
   // WV_CHECK_RET( WV_FILE_Access(pFileName)); 
    //sprintf( pDev-> decFileName,"%s", pFileName); 
...
WV_S32 HIS_GO_RefreshLayer(HIS_DEV_HIGO_E * pDev)
{
 ...
 WV_S32 ret;
 HIGO_BLTOPT_S BltOpt = {0};
  // WV_CHECK_RET( HI_GO_WaitForBlank(pDev -> layerHndl) );  
  // WV_CHECK_RET(HI_GO_RefreshLayer(pDev -> layerHndl, NULL)); 
   ret = HIS_FB_SurfGet(screenSurf);
   if(ret == WV_EFAIL)
   {
     return WV_EFAIL;
   }
    //BltOptKey.EnableScale = HI_TRUE;
    //BltOpt.EnablePixelAlpha = HI_TRUE;
    //BltOpt.PixelAlphaComp =  HIGO_COMPOPT_SRC;
    HI_RECT Rect;
    ...
    Rect.w = 1920;
    Rect.h = 1080+4;
	// WV_printf(" handl screenSurf = 0x%x pDev->layerSurfHndl =  0x%x\r\n",screenSurf[0],screenSurf[1]);   
  // WV_CHECK_RET( HI_GO_FillRect(screenSurf[0],NULL, 0x0, HIGO_COMPOPT_NONE) );
    ...
    Rect.w = 1920;
    Rect.h = 1080+4;
    // WV_CHECK_RET( HI_GO_FillRect(screenSurf[1],NULL, 0x0, HIGO_COMPOPT_NONE) );
    WV_CHECK_RET( HI_GO_Blit(pDev->layerSurfHndl,&Rect,screenSurf[1], NULL, &BltOpt)); 
