...
#define HIS_IIC_DEBUG_MODE 
  ...
  #define HIS_IIC_printf(...)  do { printf("\n\rHIS-IIC:" __VA_ARGS__); fflush(stdout); } while(0)
#else 
...
WV_S32 HIS_IIC_GetReg(WV_S32 argc, WV_S8 **argv, WV_S8 * prfBuff)
{
  ...
  if(argc<2) 
  {
   	prfBuff += sprintf(prfBuff,"get iic <BusId> <DevAddr> <RegAddr> <DataNum> <AddrNum> \r\n");
    return 0;
  }
...
WV_S32 HIS_IIC_SetReg(WV_S32 argc, WV_S8 **argv, WV_S8 * prfBuff)
{
  ...
  if(argc<4) 
  {
   	prfBuff += sprintf(prfBuff,"set iic <BusId> <DevAddr> <RegAddr> <Data> <AddrNum>\r\n");
    return 0;
  }
  ret =  WV_STR_S2v(argv[0],&busId);
  if(ret != WV_SOK)
