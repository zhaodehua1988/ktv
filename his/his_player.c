...
static WV_S32 HIS_PLAYER_CallBack(HI_HANDLE hPlayer, HI_SVR_PLAYER_EVENT_S *pstruEvent)
{
    ...
    else if (HI_SVR_PLAYER_EVENT_EOF == (HI_SVR_PLAYER_EVENT_E)pstruEvent->eEvent)
    {
        ...
        }else if(TSK_SCENE_GetSyncEna() == 1){
    ...
    else if (HI_SVR_PLAYER_EVENT_ERROR == (HI_SVR_PLAYER_EVENT_E)pstruEvent->eEvent)
    {
        ...
        {
            //WV_printf("\e[31m ERR: Time Out! \e[0m \n");
...
WV_S32  HIS_PLAYER_Start(HI_HANDLE  * pHndlPlayer ,WV_U8 * pFileName)
{
    ...
    if(TSK_SCENE_GetSyncEna() == 0)
    {
        ...
        {
            HI_SVR_PLAYER_Play(*pHndlPlayer);
            WV_ERROR(" play fail, ret = 0x%x  \n", s32Ret);
        }
        //printf("In HIS_PLAYER_Start TSK_SCENE_GetSyncEna() == 0 HI_SVR_PLAYER_Play success ++++++\n");
    }
...
WV_S32  HIS_PLAYER_PlayRandom(HI_HANDLE  *pHndlPlayer)
{
    ...
    WV_printf("get player random id=%d\n",id);

    for(i=0;i<128;i++)
    {
        memset(mov,0,sizeof(mov));
        sprintf(mov,"./mov/mov%d.mp4",i);
        ret = WV_FILE_Access(mov);
        if(ret == WV_SOK) y=i;

    }
    WV_printf("max mov id = %d \n",y);
    srand(time(0));
    id=x+rand()%(y-x+1);
    ...
    for(id;id<=y;id++)
    {
        ...
        if(loopNum == 2)
        {
            WV_printf("player playRandom loopNum > 2 \n");
            break;
        }
        ret = WV_FILE_Access(mov);
        if(ret == WV_SOK)
        {
            HIS_PLAYER_Stop(pHndlPlayer);
            HIS_PLAYER_Start(pHndlPlayer ,mov);
            //printf("auto change mov to %s \n",mov);
            return WV_SOK;
        }
    }
    return WV_SOK;
