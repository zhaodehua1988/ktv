...
//////////////////////////////////////////////
#define TSK_UART_QUE_BUFLEN 1024
#define TSK_UART_QUE_DEPTH 10
typedef struct TSK_UART_CMD_QUE_S
{
	WV_U8 *pQueBuf;
	WV_U8 *pBuf1;
	WV_QUE_DEV_E queRx;
...
static TSK_UART_CMD_QUE_S gUartCmdQue;
...
WV_S32 TSK_UART_SceneStrlApp(WV_S32 cmd)
{
	...
	case 3:
		...
		break;
	case 4:
		TSK_SCENE_PlayerVolUp();
		break;
	case 5:
		TSK_SCENE_PlayerVolDown();
		break;
	case 6:
		TSK_SCENE_PlayerStop();
		break;
	case 7:
		TSK_SCENE_StartingUP(TSK_SCENE_TYPE_UARTDATA);
		break;
	case 8:
		TSK_SCENE_Standby(TSK_SCENE_TYPE_UARTDATA);
		break;
	case 9:
		TSK_SCENE_LockScene();
		break;
...
WV_S32 tsk_uart_QueInit()
{
	...
	gUartCmdQue.pQueBuf = (WV_U8 *)malloc(TSK_UART_QUE_BUFLEN * TSK_UART_QUE_DEPTH);
	memset(gUartCmdQue.pQueBuf,0,TSK_UART_QUE_BUFLEN * TSK_UART_QUE_DEPTH);

	WV_CHECK_RET(WV_QUE_Create(&(gUartCmdQue.queRx), TSK_UART_QUE_DEPTH));
	WV_CHECK_RET(WV_QUE_Create(&(gUartCmdQue.queFree), TSK_UART_QUE_DEPTH));

	pBuf = (WV_U32)(gUartCmdQue.pQueBuf);
	for (i = 0; i < TSK_UART_QUE_DEPTH; i++)
	{
		WV_CHECK_RET(WV_QUE_Put(&(gUartCmdQue.queFree), (WV_U32) (gUartCmdQue.pQueBuf + TSK_UART_QUE_BUFLEN * i), 0));
...
/*************************************************************************************
 * 
 * WV_S32 tsk_uart_QueDeInit()
 * 
 * ***********************************************************************************/
WV_S32 tsk_uart_QueDeInit()
{
	WV_S32 ret=0;
	ret|=WV_QUE_Destroy( &gUartCmdQue.queRx );  
	ret|=WV_QUE_Destroy( &gUartCmdQue.queFree);  
	free(gUartCmdQue.pQueBuf);
	return ret;
}
/*************************************************************************************
 * 
 * WV_S32 tsk_uart_QueGet()
 * 
 * ***********************************************************************************/
WV_S32 TSK_UART_QueGet(WV_U8 *pBufOut, WV_U32 *pLenOut)
{

	//printf("get uart val from que\n");
	WV_S32 ret = 0;
	WV_U8 *pQue;

	ret = WV_QUE_Get(&gUartCmdQue.queRx, (WV_U32 *)(&pQue), 0);
	if (ret != WV_SOK)
	{
		return WV_EFAIL;
	}
	*pLenOut = *((WV_U32 *)pQue);
	memcpy(pBufOut, pQue + 4, *pLenOut);
	ret = WV_QUE_Put(&gUartCmdQue.queFree, (WV_U32)pQue, 0);
	return ret;
}
/*************************************************************************************
 * 
 ...
 * 
 * ***********************************************************************************/
WV_S32 tsk_uart_QueSet(WV_U8 *pBufIn, WV_S32 len)
{
	//printf("set uart val to que\n");
	WV_S32 ret = 0;
	WV_U8 *pQue;
	ret = WV_QUE_Get(&gUartCmdQue.queFree, (WV_U32 *)(&pQue), 0);
	if(ret != WV_SOK)
	{
		return WV_EFAIL;
	}
	*((WV_U32 *)pQue) = len;
	memcpy(pQue + 4, pBufIn, len);
	ret = WV_QUE_Put(&gUartCmdQue.queRx, (WV_U32)pQue, 0);
	return ret;
}
